...
class AppiumDriver extends BaseDriver {
  ...
  async createSession (jsonwpCaps, reqCaps, w3cCapabilities) {
    ...
    try {
      ...
      // ADDED BY MO: deviceUDID 별로 session을 관리할 수 있도록 기능 추가
      } else if (this.args.sessionOverrideByUDID) {
        await this.deleteAllSessions(desiredCaps);
      }
      // END
  ...
  // MODIFIED BY MO: udid값이 있는경우 udid용 session만 cleanup 한다.
  // async deleteSession (sessionId) {
  async deleteSession (sessionId, udid = null) {
  //END
    let protocol;
    try {
      ...
      await sessionsListGuard.acquire(AppiumDriver.name, () => {
        ...
        // ADDED BY MO: udid값이 있는경우 udid용 session만 cleanup 한다.
        if (udid &&
          this.sessions[sessionId].caps &&
          this.sessions[sessionId].caps.udid &&
          udid !== this.sessions[sessionId].caps.udid) {
          log.error(`The --session-override-by-udid option is activated. Do not delete session ${sessionId} for device ${udid}.`);
          return;
        }
        // END
  ...
  async deleteAllSessions (opts = {}) {
    ...
    // MODIFIED BY MO: opts에 udid값이 있는경우 udid용 session만 cleanup 한다.
    // const {
    //   force = false,
    //   reason,
    // } = opts;
    ...
    // MODIFIED BY MO: opts에 udid값이 있는경우 udid용 session만 cleanup 한다.
      ...
      : _.keys(this.sessions).map((id) => this.deleteSession(id, udid));
    // END
